...
<Container>
  	<Content>
		...
		Let's start by showcasing ABM Sheets on an example simulation that models the shooting of cannonballs in a two-dimensional space.

		Live demo of the model can be found on [this link](https://abm-sheets-web.vercel.app/spreadsheet?projectId=6b2f15b3-b6cb-459d-ac54-d4682d49a792).
		...
		Then there is a constant value of gravity labeled `[@G]`.

		The physics simulation of a cannonball is modeled as following:

		- `[@V] = RANDBETWEEN(90, 120)`
		- `[@A] = RANDBETWEEN(20, 45)`
		- `[@X] = [@V] * COS(RADIANS([@A])) * STEP()`
		- `[@Y] = [@V] * SIN(RADIANS([@A])) * STEP() - 0.5 * [@G] * POWER(STEP(), 2)`

		For each cannonball, the x and y values are rendered in the graph to show the shooting curve of the corresponding cannonball.
